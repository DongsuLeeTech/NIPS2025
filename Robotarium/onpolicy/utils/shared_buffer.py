...
class SharedReplayBuffer(object):
    ...
    def __init__(self, args, num_agents, obs_space, cent_obs_space, act_space):
        ...
        self.comm_graphs = np.zeros((self.episode_length + 1, self.n_rollout_threads, num_agents, num_agents), dtype=np.float32)

        self.step = 0

    def insert(self, share_obs, obs, rnn_states_actor, rnn_states_critic, actions, action_log_probs,
               value_preds, rewards, masks, bad_masks=None, active_masks=None, available_actions=None, comm_graphs=None):
        """
        ...
        if comm_graphs is not None:
            self.comm_graphs[self.step + 1] = comm_graphs.copy()
    ...
    def after_update(self):
        ...
        self.comm_graphs[0] = self.comm_graphs[-1].copy()
    ...
    def recurrent_comm_generator(self, advantages, num_mini_batch, data_chunk_length):
        """
        Yield training data for chunked RNN training with communication protocol.
        :param advantages: (np.ndarray) advantage estimates.
        :param num_mini_batch: (int) number of minibatches to split the batch into.
        :param data_chunk_length: (int) length of sequence chunks with which to train RNN.
        """
        
        # each sampled date should include all agents from the same env at the same time step to support communication
        ...
        for indices in sampler:
            ...
            masks_batch = np.stack(masks_batch, axis=1)
            active_masks_batch = np.stack(active_masks_batch, axis=1)
            comm_graphs_batch = np.stack(comm_graphs_batch, axis=1)
            old_action_log_probs_batch = np.stack(old_action_log_probs_batch, axis=1)
            adv_targ = np.stack(adv_targ, axis=1)
