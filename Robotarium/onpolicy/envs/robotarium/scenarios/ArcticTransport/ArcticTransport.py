...
from onpolicy.envs.robotarium.scenarios.ArcticTransport.visualize import Visualize
from onpolicy.envs.robotarium.utilities.roboEnv import roboEnv
from onpolicy.envs.robotarium.scenarios.ArcticTransport.agent import Agent

class ArcticTransport(BaseEnv):
    def __init__(self, args):
        self.args = args
        self.num_robots = self.args.num_agents
        self.num_agent = self.num_robots
        ...
        self.shared_observation_space = spaces.Tuple(tuple([spaces.Box(low=-1.5, high=1.5,
                                                                       shape=(self.agent_obs_dim * self.num_agent,),
                                                                       dtype=np.float32)]))
