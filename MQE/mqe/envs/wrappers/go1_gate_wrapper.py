...
class Go1GateWrapper(EmptyWrapper):
    def __init__(self, env):
        ...
        self.observation_space = spaces.Box(low=-float('inf'), high=float('inf'), shape=(14 + self.num_agents,),
                                            dtype=float)
        self.share_observation_space = spaces.Box(low=-float('inf'), high=float('inf'), shape=(62,),
                                            dtype=float)
        self.action_space = spaces.Box(low=-1, high=1, shape=(3,), dtype=float)
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.action_scale = torch.tensor([[[2, 0.5, 0.5], ], ], device=self.device).repeat(self.num_envs, self.num_agents, 1)
    ...
    def reset(self, use_privileged_obs=True):
        ...
        if use_privileged_obs:
            ...
            privileged_obs_buf = torch.cat([quat, lin_vel, ang_vel, dof_pos, dof_vel, last_act], dim=1).reshape([self.env.num_envs, self.env.num_agents, -1])
    ...
    def step(self, action, use_privileged_obs=True):
        ...
        crossed = base_pos[:, 0] > self.gate_distance + 0.25
        new_success = crossed & (~self._succeeded.view(-1))  # shape: (envs*agents,)
        self._succeeded.view(-1)[new_success] = True
        # success reward
        if self.success_reward_scale != 0:
            success_reward = torch.zeros_like(base_pos[:, 0])
            success_reward[crossed] = self.success_reward_scale
            reward += success_reward.view([self.num_envs, self.num_agents])
