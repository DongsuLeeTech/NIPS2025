...
from mqe.algorithms.utils.rnn import RNNLayer
from mqe.algorithms.utils.act import ACTLayer
from mqe.algorithms.utils.scheduler import Scheduler
from mqe.algorithms.utils.transformer_comm import Transformer_Comm

from mqe.utils.util import get_shape_from_obs_space
...
class ImIWoL_Actor(nn.Module):
    def __init__(self, config, obs_space, cent_obs_space, action_space, device=torch.device("cpu")):
        ...
        if type(sch_shape) == int:
            self.act = ACTLayer(action_space, self.latent_size + sch_shape, self._use_orthogonal, self._gain, self.config)
        else:
            self.act = ACTLayer(action_space, self.latent_size + sch_shape[0], self._use_orthogonal, self._gain, self.config)
