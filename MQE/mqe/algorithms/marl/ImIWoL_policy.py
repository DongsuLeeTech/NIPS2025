import torch
from mqe.algorithms.marl.ImIWoL_net import ImIWoL_Actor, ImIWoL_Critic
from mqe.utils.util import update_linear_schedule
...
class ImIWoL_Policy:
    def __init__(self, config, obs_space, cent_obs_space, act_space, device=torch.device("cpu")):
        ...
        self.actor = ImIWoL_Actor(config, self.obs_space, self.share_obs_space, self.act_space, self.device)
        self.critic = ImIWoL_Critic(config, self.obs_space, self.device)
