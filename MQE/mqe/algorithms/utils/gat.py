...
class GraphAttention(nn.Module):
    ...
    def forward(self, input, adj):
        ...
        # input (tensor): [batch_size, N, in_features]
        ...
        N = adj.size()[1]
        h = torch.matmul(input, self.W).view(batch_size, N, self.num_heads, self.out_features)
        ...
        for head in range(self.num_heads):
            # coeff_i, coeff_j (tensors): intermediate matrices to calculate unnormalized coefficients [batch_size, N, 1]
            ...
            coeff = coeff_i.expand(batch_size, N, N) + coeff_j.transpose(1, 2).expand(batch_size, N, N)
        ...
        # adj: [batch_size, N, N, num_heads]
        adj = adj.unsqueeze(-1).expand(batch_size, N, N, self.num_heads)
        # attention (tensor): the matrix of coefficients used for the message aggregation [batch_size, N, N, num_heads]
        attention = e * adj
        ...
        if self.normalize:
            if self.self_loop_type != 1:
                attention += 1e-15
            attention = attention / attention.sum(dim=2).unsqueeze(dim=2).expand(batch_size, N, N, self.num_heads)
            attention = attention * adj
        # dropout on the coefficients  
        attention = F.dropout(attention, self.dropout, training=self.training)
        
        # output (tensor): the matrix of output of the gat layer [batch_size, N, num_heads*out_features]
        output = []
        for head in range(self.num_heads):
            # h_prime (tensor): [batch_size, N, out_features]
