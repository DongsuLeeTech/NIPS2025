...
class mqe_openrl_wrapper(gym.Wrapper):
    ...
    def reset(self, use_privileged_obs=False, **kwargs):
        """Reset all environments."""
        obs, state = self.env.reset()
        if use_privileged_obs:
            return obs.cpu().numpy(), state.cpu().numpy()
        else:
            return obs.cpu().numpy()

    def step(self, actions, use_privileged_obs=False, extra_data: Optional[Dict[str, Any]] = None):
        """Step all environments."""
        actions = (0.5 * actions).clip(-1, 1)
        obs, state, reward, termination, info = self.env.step(actions)
...
class SingleAgentWrapper(gym.Wrapper):
    ...
    def step(self, actions, extra_data: Optional[Dict[str, Any]] = None):
        ...
        return obs.reshape(self.num_envs, 1, -1), reward.reshape(self.num_envs, 1), torch.stack(
            [termination, termination], dim=1).reshape(self.num_envs), info
...
def get_args():
    ...
    custom_parameters = [
        ...
        {"name": "--checkpoint", "type": str,
         "help": "Saved model checkpoint path. Overrides config file if provided."},
        ...
        {"name": "--train_timesteps", "type": int,
         "help": "Maximum number of training time steps. Overrides config file if provided."},
