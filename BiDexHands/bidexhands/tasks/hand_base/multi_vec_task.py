...
class MultiVecTask():
    def __init__(self, task, rl_device, clip_observations=5.0, clip_actions=1.0):
        ...
        if isinstance(rl_device, str) and "cuda" in rl_device and ":" not in rl_device:
            self.rl_device = f"{rl_device}:{task.device_id}"
...
class SharedMultiVecTaskPython(MultiVecTask):
    def get_state(self):
        return torch.clamp(self.task.states_buf, -self.clip_obs, self.clip_obs).to(self.rl_device)

    def step(self, actions):
        # actions is already in the format [n_rollout_threads, num_agents * action_dim]
        # No need to concatenate since actions are already in the correct format
        actions_tensor = torch.clamp(actions, -self.clip_actions, self.clip_actions)
    ...
    def reset(self):
        # Initialize actions in shared format
        actions = 0.01 * (1 - 2 * torch.rand([self.task.num_envs, self.task.num_actions * 2], 
                                           dtype=torch.float32, 
