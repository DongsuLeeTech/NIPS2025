...
class BaseTask():
    ...
    def safe_reset_idx(self, env_ids):
        """
        Safely reset specified environment indices with proper CUDA tensor handling.
        Args:
            env_ids: Tensor or array containing environment ids to reset
        Returns:
            Properly formatted env_ids tensor
        """
        # Handle empty env_ids
        if isinstance(env_ids, torch.Tensor) and env_ids.shape[0] == 0:
            return torch.tensor([], device=self.device, dtype=torch.long)
            
        # Convert to tensor if not already
        if not isinstance(env_ids, torch.Tensor):
            env_ids = torch.tensor(env_ids, dtype=torch.long, device=self.device)
        elif env_ids.device != self.device:
            env_ids = env_ids.to(device=self.device)
            
        # Ensure proper format for CUDA operations
        env_ids = env_ids.contiguous().long()
        
        # Create a mask for safe indexing
        mask = torch.zeros(self.num_envs, dtype=torch.bool, device=self.device)
        mask.scatter_(0, env_ids, True)
        
        return env_ids

    def reset_idx(self, env_ids):
        """
        Reset specified environment indices.
        Args:
            env_ids: Tensor containing environment ids to reset
        """
        return self.safe_reset_idx(env_ids)
