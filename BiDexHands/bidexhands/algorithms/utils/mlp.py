...
class MLPLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(self._layer_N):
            # Clone the input for each layer to avoid in-place operation issues
            x_in = x.clone()
            x = self.fc2[i](x_in)
...
class MLPBase(nn.Module):
    ...
    def forward(self, x):
        if self._use_feature_normalization:
            # Clone input before normalization to avoid in-place operation issues
            x_in = x.clone()
            x = self.feature_norm(x_in)
