...
MAIntersectionToyConfig = dict(
    ...
    num_agents=8,
    map_config=dict(exit_length=26, lane_num=1),
...
class MAIntersectionSpawnManager(SpawnManager):
    ...
    def update_destination_for(self, agent_id, vehicle_config):
        ...
        if vehicle_config["spawn_lane_index"][0] == '>>':
            ...
            # vehicle_config["spawn_longitude"] = vehicle_config["spawn_longitude"] + 9
            vehicle_config["spawn_longitude"] = vehicle_config["spawn_longitude"] + 11.5 + np.random.uniform(-2., 2.)
            vehicle_config["destination"] = '1X1_1_'
        elif vehicle_config["spawn_lane_index"][0] == '-1X0_1_':
            # from left to right
            # if self.num_agents == 2:
            #     vehicle_config["spawn_longitude"] = 22.0
            # else:
            #     vehicle_config["spawn_longitude"] = 22.5
            # vehicle_config["spawn_longitude"] = vehicle_config["spawn_longitude"] + 19
            vehicle_config["spawn_longitude"] = vehicle_config["spawn_longitude"] + 21.5 + np.random.uniform(-2., 2.)
...
class MultiAgentIntersectionToy1Env(MultiAgentMetaDrive):
    @staticmethod
    ...
    def setup_engine(self):
        ...
        super(MultiAgentIntersectionToy1Env, self).setup_engine()
